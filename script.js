...
programStartDate: null, // When user started the 12-week program
weekStartDate: null // When current week started
>>>>>>> d81944ff6561ed3e7fb653ecac73af2aa5590208
};
...
<<<<<<< HEAD
  // Do NOT set programStartDate here. It should be set when the user
  // actually starts using the app (e.g. first workout) so counts begin
  // from user's first use rather than install/launch time.
=======
```
...
<<<<<<< HEAD
  ...
  // Only render today's workout if the user has completed onboarding
  // (provided a name). The program start date itself is set when the
  // user actually starts their first workout.
  if (appState.userName) renderTodayWorkout();
  checkMorningNotification();

  // Show landing view if we don't have a user name yet
...
<<<<<<< HEAD
  // If user hasn't completed onboarding or hasn't started the program,
  // show Week 1 and Day 1 as the default display values.
  const displayWeek =
  ...
  if (greetingEl) {
    if (appState.userName) {
      ...
      greetingEl.textContent = "Your Progressive Home Workout Journey";
    }
  }
...
function updateWeekInfo() {
  // If user hasn't finished onboarding or started the program, display
  // week/day counters as Day 1 of Week 1 and zero days in program.
...
<<<<<<< HEAD
  // Only progress weeks after onboarding (user provided name)
...
<<<<<<< HEAD
  // If this is the first real use, set the program start date and week start
  if (!appState.programStartDate) {
    appState.programStartDate = new Date().toISOString();
    // set weekStartDate to now so week progression begins from first use
    appState.weekStartDate = new Date().toISOString();
    saveState();
  }
...
<<<<<<< HEAD
  ...
  // Only perform week progression and render workouts after the user
  // has completed onboarding (provided a name). This prevents stored
  // dates from advancing the visible week before the user starts.
  if (appState.userName) {
    // Check if we need to advance to next week based on calendar
    checkWeekProgression();
    renderTodayWorkout();
...
<<<<<<< HEAD
  ...
  // clear timestamp bookkeeping but keep `timerSeconds` as the remaining value
...
<<<<<<< HEAD
  // hide all known views including landing
